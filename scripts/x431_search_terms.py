...
print('=' *110)
print(f'{"Search Term":<55} {"AG":<10} {"Imp":>6} {"Clicks":>6} {"Cost":>10} {"Conv":>5} {"Value":>10}')
print('=' *110)
total_cost =0
total_clicks =0
total_conv =0
...
count =0
for row in results:
 cost = (row.metrics.cost_micros /1000000) if row.metrics.cost_micros else0
 value = (row.metrics.conversions_value) if row.metrics.conversions_value else0
 conv = (row.metrics.conversions) if row.metrics.conversions else0
 ag_name = row.ad_group.name
 print(f'{row.search_term_view.search_term[:55]:<55} {ag_name[:10]:<10} {row.metrics.impressions:>6} {row.metrics.clicks:>6} {cost:>10.2f} {conv:>5} {value:>10.2f}')
 total_cost += cost
